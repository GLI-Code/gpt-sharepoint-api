...
def validate_api_key(auth_header: str = Security(api_key_scheme)):
    token = auth_header  # auth_header[7:]
...
async def download_file_by_path(folder: str, file_name: str, access_token: str):
    ...
    return StreamingResponse(BytesIO(resp.content), media_type="application/octet-stream", headers={
...
async def get_file(request: FileRequest) -> str:
    ...
    return await download_file_by_path(request.folder, request.fileName, token)
